...
const DirectoryTree = () => {
  ...
  const orderDivisions = (a: DivisionAggregate, b: DivisionAggregate) =>
    (a[query.sort] - b[query.sort]) * (query.direction === 'asc' ? 1 : -1);
  ...
  const handleExpandAll = () => {
    const roots = Object.keys(divisionTree).filter((key) =>
      divisionAggs?.find((div) => div.key === key)
    );
  ...
  if (selectedDivisions.size == Object.keys(divisionMap).length)
    allChecked = 'checked';
  else if (selectedDivisions.size > 0) allChecked = 'indeterminate';

  const isFiltered = (key: string) =>
    !divisionFilter || match(divisionMap[key].name, divisionFilter).length > 0;
  const isDirFiltered = (key: string) =>
    (divisionTree[key] ?? []).concat([key]).filter(isFiltered).length > 0;
  const isExpanded = (key: string) => expanded.includes(key);
  const isExpandedOrFiltered = (key: string) =>
    isExpanded(key) || (divisionFilter && isDirFiltered(key));
      ...
      <ScrollableDiv>
        ...
        >
          {!divisionAggs || isLoading ? (
            ...
            >
              {divisionAggs
                .filter((div) => isDirFiltered(div.key) && divisionMap[div.key])
                .map((div) => (
                  <DirectoryEntry
                    key={div.key}
                    nodeId={div.key}
                    ...
                    name={
                      <Highlight
                        value={divisionMap[div.key].name}
                      ...
                      ?.map((div2) => (
                        <DirectoryEntry
                          ...
                          name={
                            div.key !== div2.key ? (
                              <Highlight
                                value={divisionMap[div2.key].name}
                                query={divisionFilter}
                              />
                            ) : (
                              'Other'
                          ...
                          amount={div2.amount}
                          checked={
                            selectedDivisions.has(div2.key)
                              ? 'checked'
                              : 'unchecked'
                          }
                          onCheck={handleCheck}
