...
export const Row = styled(Box)<RowStyles>(
  ({ theme, tooltip, checkable, selected, nohover, scrollOffset }) => `
  flex-grow: 1;
  cursor: pointer;
  display: grid;
  padding-right: ${scrollOffset ?? 0}px;
  grid-template-columns: ${
    (checkable ? '[checkbox] auto ' : '') +
    '[name] minmax(0, 1fr) [count] 4em [amount] 4em'
  };
  border-top: ${tooltip ? `1px solid ${theme.palette.grey[200]}` : 'none'};
  background-color: ${selected ? theme.palette.action.selected : 'white'};
  &:hover {
    background-color: ${
      nohover ? theme.palette.common.white : theme.palette.action.hover
    };
  }
`
);
...
export const Column = styled(Box)<ColumnStyles>(
  ({ theme, column }) => `
  height: 3em;
  grid-column: ${column};
  display: flex;
...
export const NumberColumn = styled(Column)<ColumnStyles & TextStyles>(
...
const DivisionRow = forwardRef((props: RowProps, ref) => (
  ...
  >
    ...
    {props.cells.map((c) => (
      ...
      >
        <Typography>{formatColumn(c)}</Typography>
      </NumberColumn>
    ))}
  </Row>
));
