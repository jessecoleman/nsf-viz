...
export default class BarChart {
  ...
  constructor(props: ChartProps) {
    ...
    this.chartWidth =
      props.dimensions.width - this.padding.left - this.padding.right;
    // TODO better formula
    this.chartHeight =
      props.dimensions.height -
      this.padding.top -
      2 * this.padding.bottom -
  ...
  getXAxis = (scale: d3.ScaleBand<number>) =>
    d3.axisBottom<number>(scale).tickFormat(this.tickFormat.x);

  getYAxis = () =>
    d3.axisLeft<number>(this.y).tickFormat(this.tickFormat.y).ticks(5);
  ...
  redraw = () => {
    ...
    const stacks = this.chart
      .selectAll<SVGGElement, Series>('.bars')
        ...
        (exit) => {
          exit.each((div, i, stack) => {
            d3.select<SVGGElement, Series[]>(stack[i])
              ...
              .attr('height', 0)
              .attr('y', (d, i) =>
                this.getYTransition(div.key, d, i, this, this.prev)
              )
              .remove();
          });
          exit.transition().duration(this.animationDur).remove();
        }
      );
    ...
    stacks.each((div, i, stack) => {
      const bars = d3
        .select<SVGGElement, Series[]>(stack[i])
        .selectAll<SVGRectElement, Series>('.bar')
        .data(
          (d) => d,
          (d) => d.data.year
        ...
        .join(
          (enter) =>
            enter
              ...
              .attr(
                'width',
                this.prev ? this.prev.x.bandwidth : this.x.bandwidth()
              )
              ...
              .attr('y', (d, i) =>
                this.getYTransition(div.key, d, i, this.prev, this)
              )
  ...
  getStackOrderOffsets = () => {
    if (this.prev && this.prev.divs.length !== this.divs.length) {
      // determine whether groups were added or removed
      const [sub, sup] =
        this.prev.divs.length < this.divs.length
          ? [this.prev.divs, this.divs]
          : [this.divs, this.prev.divs];

      return Object.fromEntries(
        sup.filter((s) => !sub.includes(s)).map((s, i) => [s, i])
      );
    } else {
  ...
  highlightGroup = (group?: string) => {
    d3.selectAll('.bars').selectAll('.bar').classed('selected', false);
