...
const ChartLegend = () => {
  ...
  const { ref: togglesRef, active: chartTogglesActive } =
    useWizardRef<HTMLButtonElement>('chartToggles');
  const { ref: anyAllRef, active: anyAllActive } =
    useWizardRef<HTMLButtonElement>('anyAllSetting');
          ...
          >
            {units.map((u) => (
              <FormControlLabel
                key={u.value}
                value={u.value}
                control={<Radio color='primary' onChange={handleChangeAgg} />}
                label={
                  <Label>
                    <u.icon fontSize='small' />
                    {u.label}
                  </Label>
                }
              />
          ...
          >
            {radio.map((r) => (
              <FormControlLabel
                key={r.value}
                value={r.value}
                control={<Radio color='primary' onChange={handleChangeBool} />}
                label={r.label}
              />
            ))}
          </RadioGroup>
